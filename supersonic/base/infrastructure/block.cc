...
vector<vector<rowcount_t>> View::ColumnPieceVisitTimes(SingleSourceProjector* key_projector) const {
	vector<const vector<shared_ptr<ColumnPiece>>*> vector_of_column_piece;
	rowcount_t min_piece_size = -1;
	const BoundSingleSourceProjector* bound_key_projector = key_projector->Bind(schema_).release();
	for(int i = 0; i < column_count(); i++) {
		const vector<shared_ptr<ColumnPiece>>* cpv_ptr = &(column(i).column_piece_vector());
		if(cpv_ptr != NULL && cpv_ptr->size() < min_piece_size) {
	...
	vector<vector<rowcount_t>> result(min_piece_size, 
			vector<rowcount_t>(bound_key_projector->result_schema().attribute_count(), 0));
	for(rowcount_t i = 0; min_piece_size != -1 && i < min_piece_size; i++) {
		for(int j = 0; j < vector_of_column_piece.size(); j++) {
			result[i][j] = *vector_of_column_piece[j]->at(i)->visit_times();
		}
	}
	return result;
}
