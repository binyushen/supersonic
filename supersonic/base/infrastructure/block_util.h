	...
	class FileBuffer {
		...
		private:
			...
			string AllocateFileName(){
				...
				string prefix = "column_piece_";
				string suffix =  ".temp";
				string result = directory + prefix + std::to_string(fileNameNum) + suffix;
				return result;
			}
			// Init the buffer_.
			void Init() {
	...
	class ColumnPiece {
		public:
			...
			ColumnPiece(const ColumnPiece& source_column_piece,
				...
				file_buffer_(nullptr),
				visit_times_(source_column_piece.visit_times()) {
			...
			rowcount_t* const visit_times() const{
				return visit_times_;
			}
		private:
			...
			// Record how many times this piece visited.
			rowcount_t* const visit_times_;
