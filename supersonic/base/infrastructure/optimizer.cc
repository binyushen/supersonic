...
namespace supersonic {
			...
			for(int row_group_id = 0; row_group_id < row_group_count_; row_group_id ++) {
				...
				for(int i = 0; i < column_count_; i++) {
					if(storage_map_[row_group_id][value_order_[query_id][row_group_id][i]]) 
						filter_order_[query_id][row_group_id][pointer++] = value_order_[query_id][row_group_id][i];
				}
				for(int i = 0; i < column_count_; i++) {
					if(!storage_map_[row_group_id][value_order_[query_id][row_group_id][i]]) 
						filter_order_[query_id][row_group_id][pointer++] = value_order_[query_id][row_group_id][i];
	...
	double Optimizer::ioCostOfOneQuery(int query_id, int row_group_id, const vector<bool>& in_memory) {
		...
		for(int i = 0; i < column_count_; i++) {
			if(in_memory[value_order_[query_id][row_group_id][i]]) accumulate_selectivity *= selectivity(query_id, row_group_id, value_order_[query_id][row_group_id][i]);
		}
		double disk_selectivity = 1.0;
		for(int i = 0; i < column_count_; i++) {
			if(!in_memory[value_order_[query_id][row_group_id][i]]) {
	...
	void Optimizer::SubsetSumRecursive(int row_group_id,
		...
		if(s == target)
		{
			...
			double curr_value = value(row_group_id, temp_in_memory);
		...
		for (int ai = 0; ai < column_width.size(); ai++) {
			...
			SubsetSumRecursive(row_group_id, remaining, remaining_position, target, partial_rec, partial_rec_position, max_value, in_memory);
		}
	}
	...
	void Optimizer::FillKnapsack() {
		...
		for(int row_group_id = 0; row_group_id < row_group_count_; row_group_id++) {
			...
			for(int i = 1; i <= column_width_sum; i++) {
				...
				SubsetSumRecursive(row_group_id, *column_width_, position, i, vector<int>(), vector<int>(), value, in_memory);
				if(value > 0.0) {
					group_knapsack.push_back(Items(i, in_memory, value)) ;
				}
			}
